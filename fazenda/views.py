...
def home(request):
    ...
    context = {
        ...
        "message": message,
        "num_images": models.FotoOrdenha.objects.count(),
    }
    ...
    if request.method == "POST":
        ...
        if upload_form.is_valid():
            ...
            if uploaded_file.content_type in {"image/jpeg", "image/png"}:
                ...
                # Blank labels
                labels_model = models.Labels(creation_date=datetime.date.today())
                labels_model.save()

                image_model = models.FotoOrdenha(labels=labels_model)
                image_model.original.save(
                    os.path.basename(uploaded_file.name), uploaded_file
                )
                image_model.save()
                ...
                for filename_and_ext in z.namelist():
                    ...
                    if ext.lower() in {".png", ".jpg", ".jpeg"}:
                        entry = name_to_image_and_labels.setdefault(
                            filename, dict(image=None, labels=None)
                        )
                                    ...
                                    with z.open(entry["labels"], mode="r") as f_labels:
                                        try:
                                            ...
                                            (
                                                labels,
                                                complete,
                                            ) = labelling_tool.PersistentLabelledImage._unwrap_labels(
                                                wrapped_labels
                                            )
                                            complete = (
                                                complete
